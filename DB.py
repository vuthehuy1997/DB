...
class DB():
    def __init__(self):
        cmd = dict()
        cmd['exp'] = 'experiments/seg_detector/cavet_resnet50_deform_thre.yaml'
        # cmd['resume'] = 'model/final_totaltext_50_sanh_train_mix'
        cmd['resume'] = 'weight/totaltext_r50'
        cmd['box_thresh'] = 0.5
        # cmd['thresh'] = 0.5
        cmd['polygon'] = False
        cmd['visualize'] = True
    ...
    def predict_image(self, image):
        ...
        with torch.no_grad():
            ...
            # if not self.args['polygon']: 
            #     for i in range(len(output[0][0])):
            #         output[0][0][i] = self.min_area(output[0][0][i])
            ...
            for index in range(batch['image'].size(0)):
                ...
                if self.args['polygon']:
                    ...
                    for i in range(boxes.shape[0]):
                        ...
                        result = [[int(box[x]), int(box[x+1])] for x in range(0,len(box),2)]
                        result_boxes.append(result)
                        result_scores.append(str(score))
        return {
            # 'seg': pred.tolist(),
            'boxs': result_boxes,
