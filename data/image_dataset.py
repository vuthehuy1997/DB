...
class ImageDataset(data.Dataset, Configurable):
    ...
    def __init__(self, data_dir=None, data_list=None, cmd={}, **kwargs):
        ...
        #print(self.data_list[0], self.is_training)
        #exit()
    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                ...
                gt_path=[self.data_dir[i]+'/train_gts/'+'.'.join(timg.strip().split('.')[:-1])+'.txt' for timg in image_list]
            else:
                ...
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i]:
                    ...
                    gt_path=[self.data_dir[i]+'/test_gts/'+'.'.join(timg.strip().split('.')[:-1])+'.txt' for timg in image_list]
